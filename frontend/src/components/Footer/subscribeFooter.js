import React, { useState, useCallback } from "react";
import { useNavigate } from "react-router-dom";

const SubscribeFooter = () => {
  const [formData, setFormData] = useState({ email: "" });
  const [isLoading, setIsLoading] = useState(false);
  const [message, setMessage] = useState(null);

  const navigate = useNavigate();

  // Function to display messages (success/error)
  const displayMessage = useCallback((type, text) => {
    setMessage({ type, text });
    const timer = setTimeout(() => {
      setMessage(null); // Clear message after 5 seconds
    }, 5000);
    return () => clearTimeout(timer); // Cleanup on unmount or re-render
  }, []);

  const handleChange = (e) => {
    // Only update email, as it's the only field in formData now
    setFormData({ email: e.target.value });
  };

  const handleSubmit = async (e) => {
    e.preventDefault(); // Prevent default browser form submission
    setIsLoading(true); // Set loading state to true
    setMessage(null); // Clear previous messages

    // Use a default base URL if REACT_APP_API_BASE_URL is not set
    const baseUrl = process.env.REACT_APP_API_BASE_URL;

    try {
      const response = await fetch(`${baseUrl}/email/subscribe`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        // Sending only email as requested
        body: JSON.stringify({ email: formData.email }),
      });

      const result = await response.json();

      if (response.ok) {
        displayMessage(
          "success",
          result.message || "Subscribed successfully! Check your inbox."
        );
        setFormData({ email: "" }); // Reset email field

        // Navigate to /subscribe after a short delay to allow the message to be seen
        setTimeout(() => {
          navigate("/");
        }, 1500); // Navigate after 1.5 seconds
      } else {
        // Display error message from the backend or a generic one
        displayMessage(
          "error",
          result.message || "Subscription failed. Please try again."
        );
      }
    } catch (error) {
      console.error("Subscription error:", error);
      displayMessage(
        "error",
        "Failed to subscribe. Please check your network and try again."
      );
    } finally {
      setIsLoading(false); // Always set loading state to false
    }
  };

  return (
    <div className="subscribe-footer-wrapper-container">
      {" "}
      {/* Added a wrapper for messages and form */}
      {message && (
        <div className={`message ${message.type}`}>{message.text}</div>
      )}
      <form onSubmit={handleSubmit} className="input-wrapper">
        <svg
          className="icon-subscribe"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
        >
          <g data-name="Layer 2">
            <g data-name="inbox">
              <rect
                width="24"
                height="24"
                transform="rotate(180 12 12)"
                opacity="0"
              ></rect>
              <path d="M20.79 11.34l-3.34-6.68A3 3 0 0 0 14.76 3H9.24a3 3 0 0 0-2.69 1.66l-3.34 6.68a2 2 0 0 0-.21.9V18a3 3 0 0 0 3 3h12a3 3 0 0 0 3-3v-5.76a2 2 0 0 0-.21-.9zM8.34 5.55a1 1 0 0 1 .9-.55h5.52a1 1 0 0 1 .9.55L18.38 11H16a1 1 0 0 0-1 1v2a1 1 0 0 1-1 1h-4a1 1 0 0 1-1-1v-2a1 1 0 0 0-1-1H5.62z"></path>
            </g>
          </g>
        </svg>
        <input
          type="email" // Changed to email type for better validation
          name="email" // Changed name to 'email' for consistency
          className="subscribe-input"
          placeholder="Your Email"
          value={formData.email} // Controlled component
          onChange={handleChange}
          required // Make email field required
          disabled={isLoading} // Disable input when loading
        />
        <button type="submit" className="Subscribe-btn" disabled={isLoading}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="30"
            height="10"
            viewBox="0 0 38 15"
            className="arrow"
          >
            <path d="M10 7.519l-.939-.344h0l.939.344zm14.386-1.205l-.981-.192.981.192zm1.276 5.509l.537.843.148-.094.107-.139-.792-.611zm4.819-4.304l-.385-.923h0l.385.923zm7.227.707a1 1 0 0 0 0-1.414L31.343.448a1 1 0 0 0-1.414 0 1 1 0 0 0 0 1.414l5.657 5.657-5.657 5.657a1 1 0 0 0 1.414 1.414l6.364-6.364zM1 7.519l.554.833.029-.019.094-.061.361-.23 1.277-.77c1.054-.609 2.397-1.32 3.629-1.787.617-.234 1.17-.392 1.623-.455.477-.066.707-.008.788.034.025.013.031.021.039.034a.56.56 0 0 1 .058.235c.029.327-.047.906-.39 1.842l1.878.689c.383-1.044.571-1.949.505-2.705-.072-.815-.45-1.493-1.16-1.865-.627-.329-1.358-.332-1.993-.244-.659.092-1.367.305-2.056.566-1.381.523-2.833 1.297-3.921 1.925l-1.341.808-.385.245-.104.068-.028.018c-.011.007-.011.007.543.84zm8.061-.344c-.198.54-.328 1.038-.36 1.484-.032.441.024.94.325 1.364.319.45.786.64 1.21.697.403.054.824-.001 1.21-.09.775-.179 1.694-.566 2.633-1.014l3.023-1.554c2.115-1.122 4.107-2.168 5.476-2.524.329-.086.573-.117.742-.115s.195.038.161.014c-.15-.105.085-.139-.076.685l1.963.384c.192-.98.152-2.083-.74-2.707-.405-.283-.868-.37-1.28-.376s-.849.069-1.274.179c-1.65.43-3.888 1.621-5.909 2.693l-2.948 1.517c-.92.439-1.673.743-2.221.87-.276.064-.429.065-.492.057-.043-.006.066.003.155.127.07.099.024.131.038-.063.014-.187.078-.49.243-.94l-1.878-.689zm14.343-1.053c-.361 1.844-.474 3.185-.413 4.161.059.95.294 1.72.811 2.215.567.544 1.242.546 1.664.459a2.34 2.34 0 0 0 .502-.167l.15-.076.049-.028.018-.011c.013-.008.013-.008-.524-.852l-.536-.844.019-.012c-.038.018-.064.027-.084.032-.037.008.053-.013.125.056.021.02-.151-.135-.198-.895-.046-.734.034-1.887.38-3.652l-1.963-.384zm2.257 5.701l.791.611.024-.031.08-.101.311-.377 1.093-1.213c.922-.954 2.005-1.894 2.904-2.27l-.771-1.846c-1.31.547-2.637 1.758-3.572 2.725l-1.184 1.314-.341.414-.093.117-.025.032c-.01.013-.01.013.781.624zm5.204-3.381c.989-.413 1.791-.42 2.697-.307.871.108 2.083.385 3.437.385v-2c-1.197 0-2.041-.226-3.19-.369-1.114-.139-2.297-.146-3.715.447l.771 1.846z"></path>
          </svg>
          {isLoading ? "Subscribing..." : "Subscribe"}
        </button>
      </form>
      {/* Inbuilt CSS based on your provided styles */}
      <style>{`
        .subscribe-footer-wrapper-container {
          display: flex;
          flex-direction: column;
          align-items: center;
          gap: 10px;
        }

        .input-wrapper {
          width: fit-content;
          height: 45px;
          border-radius: 20px;
          padding: 5px;
          box-sizing: content-box;
          display: flex;
          align-items: center;
          background-color: #292524;
          color: #ffffff;
        }

        .icon-subscribe {
          width: 40px;
          fill: #ffffff; /* Changed fill to black */
          margin-left: 8px;
          transition: all 0.3s;
        }
        .subscribe-input {
          max-width: 170px;
          height: 100%;
          border: none;
          outline: none;
          padding-left: 15px;
          background-color: #292524;
          color: #ffffff;
          font-size: 1em;
        }

        subscribe-input:focus {
            color: #ffffff;
        }

        .subscribe-input:-webkit-autofill {
          -webkit-box-shadow: 0 0 0px 1000px #292524 inset;
          -webkit-text-fill-color: #ffffff;
        }

        .Subscribe-btn {
          height: 100%;
          width: 95px;
          border: none;
          border-radius: 15px;
          color: rgb(0, 0, 0);
          cursor: pointer;
          background-color: #ffffff;
          font-weight: 500;
          overflow: hidden;
          display: flex;
          align-items: center;
          justify-content: center;
          position: relative;
          transition: all 0.3s;
        }
        .arrow {
          position: absolute;
          margin-right: 150px;
          transition: all 0.3s;
        }
        .input-wrapper:active .icon {
          transform: scale(1.3);
        }
        .Subscribe-btn:hover {
          color: white;
        }
        .Subscribe-btn:hover .arrow {
          margin-right: 0;
          animation: jello-vertical 0.9s both;
          transform-origin: right;
        }

        @keyframes jello-vertical {
          0% {
            transform: scale3d(1, 1, 1);
          }
          30% {
            transform: scale3d(0.75, 1.25, 1);
          }
          40% {
            transform: scale3d(1.25, 0.75, 1);
          }
          50% {
            transform: scale3d(0.85, 1.15, 1);
          }
          65% {
            transform: scale3d(1.05, 0.95, 1);
          }
          75% {
            transform: scale3d(0.95, 1.05, 1);
          }
          100% {
            transform: scale3d(1, 1, 1);
          }
        }
        .Subscribe-btn:active {
          transform: scale(0.9);
        }

        .message {
          padding: 8px 15px;
          border-radius: 5px;
          font-size: 0.9em;
          font-weight: bold;
          text-align: center;
          width: fit-content;
          max-width: 100%;
          word-break: break-word;
        }

        .message.success {
          background-color: #d4edda;
          color: #155724;
          border: 1px solid #c3e6cb;
        }

        .message.error {
          background-color: #f8d7da;
          color: #721c24;
          border: 1px solid #f5c6cb;
        }

        @media screen and (min-width: 1200px) and (max-width: 1399px) {
          .subscribe-input {
            max-width: 120px;
          }
          .Subscribe-btn {
            width: 100px;
          }
        }

        @media screen and (min-width: 992px) and (max-width: 1199px) {
          .subscribe-input {
            max-width: 80px;
            height: 80%;
            font-size:0.9em;
            padding-left: 10px;
          }
          .Subscribe-btn {
            width: 100px;
            height: 100%;
            font-size: 0.9em;
          }

          .icon-subscribe {
            width:25px;
          }

          .input-wrapper {
            height: 35px;
          }
        }

        @media screen and (min-width: 300px) and (max-width: 359px) {
          .subscribe-input {
            max-width: 50vw;
            height: 80%;
          }
          .Subscribe-btn {
            font-size: 0.9em;
            height: 100%;
            width: 95px;
            border-radius: 12px;
          }

          .icon-subscribe {
            width: 35px;
          }

          .input-wrapper {
            height: 35px;
          }
        }

        


      `}</style>
    </div>
  );
};

export default SubscribeFooter;
